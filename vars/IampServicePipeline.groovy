    ...
    stage ("Checkout & Build") {
      ...
      stage("create test route"){
       when {
        ...
        steps{
          sh "oc process -f cicd/iamp-spring-service-prod-route-template.yaml -p APP_NAME=${appName} -p API_VERSION=${apiVersion} -l commit=${cicdCommit} -l app=${appName}-${apiVersion} | oc create -f- -n ${testProject} | oc expose svc/${appName}-${apiVersion}-${version} --name=${appName}-${apiVersion} || true"
        }
      }
