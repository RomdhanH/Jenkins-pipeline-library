...
def call(Map config) {
	...
	def command = "oc process -f cicd/iamp-spring-service-template.yaml -p APP_NAME=${appName} -p IMAGE_NAME=${appName} -p IMAGE_TAG= " + config.version + " -p REPLICAS=" + config.replicas + " -l app=${appName},commit=${cicdCommit} | oc create -f- -n " + config.project + " || true"
